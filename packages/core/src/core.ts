 ...
 * `orderOnlyDeps` and `validations` that may have been generated by another rule, which makes
...
> = {
  out: O;
    ...
    [orderOnlyDeps]?: Input<string> | readonly Input<string>[];
    [validations]?: (out: O) => Input<string> | readonly Input<string>[];
    dyndep?: string;
    pool?: string;
  ...
  for (let i = 0; i < input.length; ++i) {
    ...
    if (typeof entry === "string") {
      ...
      if (orderOnlyDeps in entry) {
        const value = entry[orderOnlyDeps];
        if (typeof value === "string") {
          out[orderOnlyDeps].push(value);
        } else if (value !== undefined) {
          out[orderOnlyDeps].push(...value);
        }
      }
      if (validations in entry) {
        const value = entry[validations];
        if (typeof value === "string") {
          out[validations].push(value);
        } else if (value !== undefined) {
          out[validations].push(...value);
        }
      }
...
export class NinjaBuilder {
  ...
  get phony(): <O extends string>(args: {
    ...
    return <O extends string>(args: {
      ...
      const { file, [orderOnlyDeps]: orderOnly } = homogenize(args.in);
      this.output +=
        "build " +
        escapePath(args.out) +
        ": phony" +
        concatPaths(" ", file, orderOnly) +
        "\n";
      return args.out;
    };
  }
   ...
   *   out: needs<string>(),
   *   in: needs<readonly Input<string>[]>(), // Note `readonly` is required on arrays
   * });
   * const archive = tar({
  ...
  rule<A extends RuleArgs>(
    ...
    const ruleHomogenized = {
      [implicitDeps]: homogenize(variables[implicitDeps]),
      [orderOnlyDeps]: homogenize(variables[orderOnlyDeps]),
    };
    ...
    >(
      ...
      const homogenized = {
        in: homogenize(_in),
        [implicitDeps]: homogenize(buildVariables[implicitDeps]),
        [orderOnlyDeps]: homogenize(buildVariables[orderOnlyDeps]),
        [validations]: homogenize(buildVariables[validations]?.(out)),
      };
      ...
      let output =
        ...
        ": " +
        name +
        concatPaths(" ", homogenized.in.file) +
        concatPaths(
          " | ",
          ruleHomogenized[implicitDeps].file,
          homogenized[implicitDeps].file,
        ) +
        concatPaths(
          " || ",
          ruleHomogenized[implicitDeps][orderOnlyDeps],
          ruleHomogenized[orderOnlyDeps].file,
          ruleHomogenized[orderOnlyDeps][orderOnlyDeps],
          homogenized.in[orderOnlyDeps],
          homogenized[implicitDeps][orderOnlyDeps],
          homogenized[orderOnlyDeps].file,
          homogenized[orderOnlyDeps][orderOnlyDeps],
        ) +
        concatPaths(
          " |@ ",
          ruleHomogenized[implicitDeps][validations],
          homogenized.in[validations],
          homogenized[implicitDeps][validations],
          homogenized[validations].file,
          homogenized[validations][validations],
        ) +
        "\n";
