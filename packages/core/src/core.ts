...
export type RuleArgs = {
  ...
  in?: Placeholder<Input<string>> | Placeholder<Input<readonly string[]>>;
  pool?: string;
  restat?: 1;
  generator?: 1;
  depfile?: string;
...
/**
 * Return the input file in the specified `buildArguments`.
 * This is either `buildArguments.in.file` if it exists, or `buildArguments.in` otherwise.
 */
  ...
  rule<A extends RuleArgs>(
    ...
    return <
      const I extends BuildArgs<A> & {
        in?: Input<string> | Input<readonly string[]>;
      },
    >(
      buildVariables: I,
    ): I["out"] => {
      const { in: _in, out, ...rest } = buildVariables;
      let input: undefined | string | readonly string[];
      let extraDeps: undefined | string | readonly string[];
      let extraOrderDeps: undefined | string | readonly string[];
      if (typeof _in === "object" && "file" in _in) {
        input = _in.file;
        extraDeps = _in[implicitDeps];
        extraOrderDeps = _in[orderOnlyDeps];
      } else {
        input = _in;
      }
